...
int wcfb_example(){
    ...
    // dump(fb_enc.table, len);
    uint8_t *op, *buf;
    buf = (uint8_t *)malloc(33);
    op = (uint8_t *)malloc(49);
    int num = 0;
    // dump(ip, 16);
    set_time_start();
    ret = FEISTALBOX_wcfb_encrypt(ip, op, 33, &fb_enc, &num, iv, FEISTALBOX_ENC);
    set_time_ends();
    printf("Enc FeistalBox Spent: %f s, %lld cycles Ret: %d\n", get_clock_elapsed(), get_cycles_elapsed(), ret);
    num = 0;
    // dump(op, 16);
    set_time_start();
    ret = FEISTALBOX_wcfb_encrypt(op, buf, 49, &fb_dec, &num, iv, FEISTALBOX_DEC);
    set_time_ends();
    printf("Dec FeistalBox Spent: %f s, %lld cycles Ret: %d\n", get_clock_elapsed(), get_cycles_elapsed(), ret);
    ret = memcmp(buf, ip, 33);
    printf("DecText ?= Plaintext:\t%s\n", ret == 0 ? "OK" : "NO");
    printf("\ninput:\n");
    dump(ip, 33);
    printf("\noutput:\n");
...
int wcbc_example(){
    ...
    // dump(ip, 16);
    set_time_start();
    ret = FEISTALBOX_wcbc_encrypt(ip, op, 33, &fb_enc, iv, FEISTALBOX_ENC, WRAP_LEN);
    set_time_ends();
    printf("Enc FeistalBox Spent: %f s, %lld cycles Ret: %d\n", get_clock_elapsed(), get_cycles_elapsed(), ret);
    // dump(op, 16);
    set_time_start();
    ret = FEISTALBOX_wcbc_encrypt(op, buf, 80, &fb_dec, iv, FEISTALBOX_DEC, WRAP_LEN);
    set_time_ends();
    printf("Dec FeistalBox Spent: %f s, %lld cycles Ret: %d\n", get_clock_elapsed(), get_cycles_elapsed(), ret);
    ret = memcmp(buf, ip, 33);
    printf("DecText ?= Plaintext:\t%s\n", ret == 0 ? "OK" : "NO");
